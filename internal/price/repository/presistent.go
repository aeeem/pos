...
func (m pricePresistentRepository) GetPrices(offset, limit int64, search string) (prices []model.Price, total int64, err error) {
	total = 0
	err = m.DB.Limit(int(limit)).Offset(int(offset)).Find(&prices).Count(&total).Error
	if err != nil {
		return
	}
	return
